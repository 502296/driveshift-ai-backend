...
export function detectDominantSignals(issue, answers) {
  ...
  if (extracted.signals.overheating) {
...
export function detectComplexity(issue, dominantSignals, answers) {
  ...
  const signalCount = Array.isArray(dominantSignals)
  ...
  if (isAdvancedCase(text)) {
    return {
      ...
      minimumQuestions: 3,
      reason:
        "advanced diagnostic cases need three focused technical questions before final analysis",
    };
  }
  ...
  if (isSimpleLowRisk(text) && signalCount === 0) {
    return {
      ...
      minimumQuestions: 2,
      reason:
        "simple issue still needs two confirmation questions for a useful report",
    };
  }
  ...
  if (isSafetySensitive(text) || signalCount >= 2) {
    return {
      ...
      minimumQuestions: 3,
      reason:
        "strong symptoms need three focused diagnostic questions before final analysis",
    };
  }
